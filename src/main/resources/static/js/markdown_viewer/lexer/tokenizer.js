...
function readTokensAt(lexer, shouldEnd) {
  const tokens = [];
...
function nextToken(lexer) {
  ...
  switch (lexer.peekChar()) {
    case "#":
      return readHeader(lexer);

    case "[":
      return readLink(lexer);

    case "!":
      return readImage(lexer);

    case "*":
    case "_":
      return readTextDecorator(lexer);

    case "`":
      return readCode(lexer);
    ...
    case "-":
      const token = readListItem(lexer);
...
function readHeader(lexer) {
  ...
  const content = readTokensAt(
    lexer,
    (token) => token.kind === TokenKind.NL || token.kind === TokenKind.EOF,
  );
  ...
  return {
    ...
    weight: Math.max(Math.min(weight, 6), 0),
    content,
  };
}

function readLink(lexer) {
...
function readTextDecorator(lexer) {
  const specialChar = lexer.readChar();

  const token =
    lexer.peekChar() === specialChar
      ? readBold(lexer, specialChar)
      : readItalic(lexer, specialChar);

  if (token.kind === TokenKind.TEXT) {
    return {
      kind: TokenKind.TEXT,
      content: specialChar + token.content,
    };
  }
...
function readBold(lexer, specialChar) {
  let fullContent = lexer.readChar();
  ...
  while (
    !lexer.isEOF() &&
    !(lexer.peekChar() === specialChar && lastChar === specialChar) &&
    lexer.peekChar() !== "\n"
  ) {
...
function readItalic(lexer, specialChar) {
  let content = "";
...
function tryToReadNewLine(lexer) {
  lexer.save();
